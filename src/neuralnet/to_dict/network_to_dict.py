...
class NetworkToDict():    
    ...
    def to_dict(self, network: Network) -> NetworkDictionary:
        node_layers = self.__get_node_layers(network)
        synapse_layers = self.__get_synapse_layers(network)
    ...
    def from_dict(self, dictionary: NetworkDictionary) -> Network:
        ...
        self.__set_node_layers(network, dictionary)
        self.__set_synapse_layers(network, dictionary)        
        
        return network
        
    def clone(self, network: Network) -> Network:
        return self.from_dict(self.to_dict(network))
        
    def __get_synapse_layers(self, network: Network) -> list[list[SynapseDict]]:
    ...
    def __get_node_layers(self, network: Network) -> list[list[NodeDict]]:
    ...
    def __set_node_layers(self, network: Network, dictionary: NetworkDictionary):
        for layer_index in range(len(network.node_layers)):
            ...
            self.__set_nodes(node_layer, node_dict_layer)
                    
    def __set_nodes(self, node_layer: list[SigmoidNode], node_dict_layer: list[NodeDict]):
    ...
    def __set_synapse_layers(self, network: Network, dictionary: NetworkDictionary):
        for layer_index in range(len(network.synapse_layers)):
            ...
            self.__set_synapses(synapse_layer, synapse_dict_layer)
            
    def __set_synapses(self, synapse_layer: list[Synapse], synapse_dict_layer: list[SynapseDict]):
