...
class BlobPoller():
    ...
    def poll(self, population: PopulationDTO) -> None:
        ...
        while self.has_blobs_to_await(blobs_to_await) and (self.is_timed_out(time_started_ms) is False):
            Logger().debug("BlobPoller - Retrieving output blob ids...")
            output_ids = self.output_blob_client.get_all_ids()
            
            Logger().debug(f'BlobPoller - Found "{len(output_ids)}" output blobs blobs...')
            ...
            Logger().debug(f'BlobPoller - Sleeping for "{self.config.polling_rate_ms}" ms...')
    ...
    def update_network_in_population(self, population: PopulationDTO, blob_name: str) -> None:
        Logger().debug(f'BlobPoller - Updating network "{blob_name}" in population.')
